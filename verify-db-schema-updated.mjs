...
async function verifyColumnTypes() {
  ...
  try {
    ...
    // If the columns are numeric, we should get a foreign key error or row-level security error
    ...
    if (insertError) {
      ...
      // Check if the error is due to foreign key constraint or row-level security (expected)
      ...
      if (insertError.message && (
        ...
        console.log('The error is due to security policy or foreign key constraint, not data type mismatch');
        ...
        // Since we're not getting a type error, it's likely the columns are numeric
        console.log('Since there is no type error, the columns are likely NUMERIC');
      }
    } else {
    ...
    // Check the error message from the console log
    console.log('\nAnalyzing error message from console log...');
    const errorMessage = 'invalid input syntax for type integer: "80.3"';
    console.log(`Error message: ${errorMessage}`);
    
    if (errorMessage.includes('invalid input syntax') && errorMessage.includes('type integer')) {
      console.log('❌ Based on the error message, the columns are still INTEGER, not NUMERIC');
      console.log('Migration may have failed or not been applied');
    } else {
      console.log('✅ No type error in the message, columns may be NUMERIC');
    }
