// This script restarts the app with the updated GAF adjustment for one-time costs
import { exec } from 'child_process';

console.log('Restarting the app with updated GAF adjustment for one-time costs...');

// Run the test script first to verify the fix
exec('node test-one-time-gaf-adjustment.mjs', (error, stdout, stderr) => {
  if (error) {
