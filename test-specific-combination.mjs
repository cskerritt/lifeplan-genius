...
// Since we can't use jest.mock in ES modules, we'll create manual mocks
const mockGeoFactorsService = {
  fetchGeoFactors: async (zipCode) => {
    console.log(`Mocking fetchGeoFactors for ZIP: ${zipCode}`);
    return {
      mfr_factor: 1.2,
      pfr_factor: 1.3
    };
...
const mockCptCodeService = {
  lookupCPTCode: async (cptCode) => {
    console.log(`Mocking lookupCPTCode for CPT: ${cptCode}`);
    return {
      code: cptCode,
