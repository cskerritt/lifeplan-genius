// Script to fix the Supabase client by adding a function to handle decimal values
import pg from 'pg';
...
// Function to fix the Supabase client
async function fixSupabaseClient() {
  console.log('Starting to fix Supabase client...');
  ...
  try {
    ...
    console.log('Creating function to handle decimal values...');
    ...
    if (triggerResult.rowCount > 0) {
      ...
      await pgClient.query(`
        DROP TRIGGER IF EXISTS handle_decimal_values_trigger ON care_plan_entries;
      `);
    }
    ...
    // Create a function to convert integer to numeric
    console.log('Creating function to convert integer to numeric...');
    await pgClient.query(`
      CREATE OR REPLACE FUNCTION to_numeric(value anyelement)
      RETURNS numeric AS $$
      BEGIN
    ...
    const testItemName = `Supabase Test Item ${new Date().toISOString()}`;
    ...
    console.log('\n✅ Supabase client fix and test completed successfully!');
    
  } catch (error) {
    console.error('Error fixing Supabase client:', error);
