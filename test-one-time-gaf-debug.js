// This script tests the geographic adjustment for one-time fees with detailed debugging
import { calculateItemCosts, fetchGeoFactors } from './src/utils/calculations/costCalculator.js';

async function testOneTimeGafDebug() {
  console.log('Testing geographic adjustment for one-time fees with detailed debugging...');
  ...
  console.log('Calculating costs WITHOUT geographic adjustment...');
  ...
  console.log(JSON.stringify(costsWithoutGeo, null, 2));
  console.log('-----------------------------------');
  
  // Then, fetch geographic factors for the ZIP code
  console.log('Fetching geographic factors...');
  ...
  // Create a mock item object to simulate what would be displayed in the UI
  const mockItem = {
    id: 'test-item',
    service: 'Pain Management',
    frequency: 'one-time',
    costRange: {
      low: costsWithGeo.low,
      average: costsWithGeo.average,
      high: costsWithGeo.high
    },
    costPerUnit: baseRate,
    cptCode: cptCode,
    mfrMin: costsWithGeo.mfrCosts?.low,
    mfrMax: costsWithGeo.mfrCosts?.high,
    pfrMin: costsWithGeo.pfrCosts?.low,
    pfrMax: costsWithGeo.pfrCosts?.high,
    mfrFactor: geoFactors?.mfr_factor,
    pfrFactor: geoFactors?.pfr_factor
  };
  
  console.log('Mock item that would be displayed in the UI:');
...
testOneTimeGafDebug().catch(console.error);
