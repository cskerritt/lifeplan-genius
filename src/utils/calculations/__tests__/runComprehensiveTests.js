...
const { generateTestCombinations, generateRepresentativeTestCombinations } = require('./testDataGenerator');
const costCalculator = require('../costCalculator').default;
...
// Setup mocks
const setupMocks = () => {
  const geoFactorsService = require('../services/geoFactorsService');
  const cptCodeService = require('../services/cptCodeService');
  ...
  cptCodeService.hasMfuData = jest.fn((cptCode) => {
    return cptCode !== 'no-mfu';
  });
  
  cptCodeService.hasPfrData = jest.fn((cptCode) => {
 ...
 */
const runTests = async (useAllCombinations = false) => {
  setupMocks();
