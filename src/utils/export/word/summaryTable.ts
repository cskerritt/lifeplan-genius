...
import { calculateCategoryOneTimeTotal } from '../utils';
  ...
  data.categoryTotals.map((total, index) => {
    ...
    return new TableRow({
      children: [
        ...
        createStyledCell(
          `$${(total.total * duration).toFixed(2)}`,
          20,
...
export const createSummaryTotalRow = (data: ExportData) => {
  const duration = parseFloat(data.lifeExpectancy || '0');
  const totalAnnualCost = data.categoryTotals.reduce((sum, cat) => sum + cat.total, 0);
  const totalLifetimeCost = totalAnnualCost * duration;
